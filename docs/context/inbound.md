...
2. **Inbound `from` is normalized but stored values may not be.** [`handleInbound`](../../src/lib/inbound/handle-inbound.ts:36) calls `normalizeEmail` on the inbound address before lookup. But [`parseSetupPayload`](../../src/lib/setup-payload.ts) (read in 0.5/0.7) needs to be checked for whether stored `emailAddress` / `phoneNumber` get the same normalization. If not, a session that was set up with `User@Example.com` won't match an inbound from `user@example.com`. Flagged for verification — likely a 0.7 or Phase 1.1 fix.
