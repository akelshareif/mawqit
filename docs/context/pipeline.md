...
9. **`expiry*SentAt` markers don't reset on session renewal.** If a session is in the warning window, gets the warning, then is extended (saved again), the `expiryWarningSentAt` stays set. If the session later approaches a new expiry window, the warning won't re-fire. Phase 1 should clear those two columns on save, or re-derive the gate. Flagging — likely a Phase 1.4 schema-touch alongside the broader rework.
